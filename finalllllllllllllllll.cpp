...
class University
{
	...
	public:
		...
		void RemoveDept(string removedpt)
		{
			string depname1=removedpt;
			for(int j=0;j<currUniSize;j++)
			{
				if(UniDepartments[j].show(depname1)==true)
				{
					for(int l=j;l<currUniSize;l++)
					{
						UniDepartments[l]=UniDepartments[l+1];
...
int main()
{
	...
	switch(ch1)
	{
		...
		case '1':
		{
			...
			cout<<"  |   REMOVE DEPARTMENT (PRESS - 5) |"<<endl;
			cout<<endl;
			cin>>ch9;
			if(ch9=='1')
			{
			...
			{
				string removedpt;
				cout<<"ENTER THE NAME OF DEPARTMENT WHICH YOU WANT TO REMVOE :"<<endl;
				cin>>removedpt;
				object3.RemoveDept(removedpt);
